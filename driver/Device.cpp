...
--*/
{
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("VsMatrix: AcxDeviceInitInitialize failed: 0x%x\n", status));
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("VsMatrix: AcxDeviceInitialize failed: 0x%x\n", status));
        return status;
    }
    ...
    //
    status = CodecC_AddStaticCapture(device, &CODEC_CAPTURE_COMPONENT_GUID, &MIC_CUSTOM_NAME, &captureCircuitName);
    if (!NT_SUCCESS(status)) {
        KdPrint(("VsMatrix: CodecC_AddStaticCapture failed: 0x%x\n", status));
        return status;
    }
...
--*/
{
    ...
    //
    status = Codec_SetPowerPolicy(Device);
    if (!NT_SUCCESS(status)) {
        KdPrint(("VsMatrix: Codec_SetPowerPolicy failed: 0x%x\n", status));
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("VsMatrix: AcxDeviceAddCircuit Render failed: 0x%x\n", status));
        return status;
    }

    ASSERT(devCtx->Capture);
    status = AcxDeviceAddCircuit(Device, devCtx->Capture);
    if (!NT_SUCCESS(status)) {
        KdPrint(("VsMatrix: AcxDeviceAddCircuit Capture failed: 0x%x\n", status));
        return status;
    }
...
--*/
{
    ...
    //
    status = AcxDeviceRemoveCircuit(Device, devCtx->Capture);
    if (!NT_SUCCESS(status)) {
        KdPrint(("VsMatrix: AcxDeviceRemoveCircuit Capture failed: 0x%x\n", status));
        return status;
    }
